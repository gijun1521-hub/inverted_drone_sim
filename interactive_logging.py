...
INTERACTIVE_FIELDS = [
    ...
    "moving_mass_offset_m", "moving_mass_velocity_m_s", "moving_mass_target_m", "moving_mass_command_m", "moving_mass_saturated",
    "moving_mass_command_clipped", "moving_mass_rail_contact", "moving_mass_rate_limited", "moving_mass_acceleration_limited",
    ...
    "desired_vx", "shaped_desired_vx", "position_velocity_correction", "total_desired_vx",
    "actual_vx", "desired_ax", "ax_target", "desired_vz", "actual_vz", "desired_az", "theta_target_limited",
    "loiter_active", "loiter_braking_active", "altitude_hold_active", "throttle_deadband_active",
    "stick_x", "stick_z", "target_x_rate_cmd", "target_z_rate_cmd", "target_capture_event",
    "target_capture_count", "target_capture_pending", "target_step_event", "actual_vane_angle",
...
def interactive_row(
    ...
    row = {
        ...
        "moving_mass_command_m": float(forces.moving_mass_command_m),
        "moving_mass_saturated": int(forces.moving_mass_saturated),
        "moving_mass_command_clipped": int(forces.moving_mass_command_clipped),
        "moving_mass_rail_contact": int(forces.moving_mass_rail_contact),
        "moving_mass_rate_limited": int(forces.moving_mass_rate_limited),
        "moving_mass_acceleration_limited": int(forces.moving_mass_acceleration_limited),
        ...
        "desired_vx": float(targets.desired_vx), "shaped_desired_vx": float(targets.shaped_desired_vx),
        "position_velocity_correction": float(targets.position_velocity_correction),
        "total_desired_vx": float(targets.total_desired_vx), "actual_vx": float(state[3]),
        "desired_ax": float(targets.desired_ax), "ax_target": float(control.ax_target),
        ...
        "target_capture_event": int(targets.target_capture_event),
        "target_capture_count": int(targets.target_capture_count),
        "target_capture_pending": int(targets.target_capture_pending),
        "target_step_event": int(targets.target_step_event),
