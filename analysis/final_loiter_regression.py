...
def _audit(rows, scenario, gain):
    t,vx,target,count=_arr(rows,"time"),_arr(rows,"vx"),_arr(rows,"target_x"),_arr(rows,"target_capture_count")
    metrics=_moving_mass_metrics(type("R",(),{"rows":rows,"scenario":type("S",(),{"moving_mass_assist_gain_m_per_Nm":gain})()})())
    events=[]; failures=[]
...
def run(output_dir: Path=OUT):
    ...
    for rerun in range(2):
        ...
        for pattern,direction,scenario in _scenarios():
            for variant,gain in VARIANTS:
                result=run_headless_loiter(PROFILE,scenario,controller_overrides={**FIXED_CONTROLLER,"enable_noise":False,"random_seed":0})
                metrics,events,failures=_audit(result.rows,scenario,gain)
                row={"pattern":pattern,"direction":"positive" if direction>0 else "negative","variant":variant,"gain":gain,"passed":not failures,"failures":"; ".join(failures),**metrics}; rows_out.append(row)
    ...
    (output_dir/"summary.md").write_text("# Final LOITER regression\n\n"+f"Passed: **{passed}**. Cases: {len(summaries)}.\n",encoding="utf-8")
